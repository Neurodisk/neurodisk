...
- **Domaine** : `neurodisk.com` désormais vérifié dans Resend (DKIM + SPF + MX de retour confirmés par requête DNS ; DNS sur Cloudflare). Expéditeur retenu **`no-reply@neurodisk.com`** + Reply-To — car `neurodisk.com` n'a aucun MX de réception (une réponse de patient y serait perdue). NB : l'app tourne sur **`plateforme.neurodisk.com`** (et non `cliniqueneurodisk.com`, qui est le site vitrine) — donc domaine d'envoi et domaine d'application partagent la même racine, ce qui est optimal pour la délivrabilité. Aucun domaine n'est codé en dur : l'edge function utilise l'`origin` de la requête et le front `window.location.origin`.
