...
- **Assistant de rédaction IA** : edge function `supabase/functions/ai-redaction` (Claude `claude-opus-4-8`, SDK officiel, clé `ANTHROPIC_API_KEY` côté serveur) ; carte dans la section Lettre de référence (type lettre/résumé, patient optionnel → contexte conditions + NPRS, brouillon éditable + copier). Garde-fous anti-invention. ⚠️ Nécessite : `supabase secrets set ANTHROPIC_API_KEY=...` + `supabase functions deploy ai-redaction`.
